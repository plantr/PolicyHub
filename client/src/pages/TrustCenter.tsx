...
export default function TrustCenter() {
  ...
  const { data: findings, isLoading: findingsLoading } = useQuery<Finding[]>({ queryKey: ["/api/findings"] });
  const { data: commitmentsList, isLoading: commitmentsLoading } = useQuery<Commitment[]>({ queryKey: ["/api/commitments"] });
  const { data: articles, isLoading: articlesLoading } = useQuery<KnowledgeBaseArticle[]>({ queryKey: ["/api/knowledge-base"] });
  const { data: businessUnits, isLoading: buLoading } = useQuery<BusinessUnit[]>({ queryKey: ["/api/business-units"] });
  const { data: mappings, isLoading: mappingsLoading } = useQuery<RequirementMapping[]>({ queryKey: ["/api/requirement-mappings"] });
