...
must_haves:
  truths:
    ...
    - "Document upload flow uses TUS signed-URL pattern instead of multer FormData in Documents.tsx, DocumentDetail.tsx, and VersionDetail.tsx"
  ...
  key_links:
    ...
    - from: "client/src/pages/DocumentDetail.tsx"
      to: "client/src/lib/storage.ts"
      via: "TUS upload replaces multer FormData for version uploads"
      pattern: "uploadFileToStorage"
    - from: "client/src/pages/VersionDetail.tsx"
      to: "client/src/lib/storage.ts"
      via: "TUS upload replaces multer FormData for version re-upload"
      pattern: "uploadFileToStorage"
---
...
<task type="auto">
  ...
  <action>
    ...
    2. **Migrate document upload in DocumentDetail.tsx:**
       - `DocumentDetail.tsx` has FormData uploads at lines ~432 and ~531 (version upload and version re-upload flows)
       - Replace both `new FormData()` usages with the same TUS signed-URL pattern:
         a. POST metadata to serverless function to create/update the version record (JSON body, no file)
         b. Call `uploadFileToStorage()` from `client/src/lib/storage.ts` with the version ID
       - Remove all `new FormData()` usage
       - Import `uploadFileToStorage` from `@/lib/storage`

    3. **Migrate document upload in VersionDetail.tsx:**
       - `VersionDetail.tsx` has FormData upload at line ~171 (version file re-upload)
       - Replace `new FormData()` with TUS signed-URL pattern (same as above)
       - Import `uploadFileToStorage` from `@/lib/storage`
  ...
  <verify>
    - Grep `Documents.tsx` for `FormData` — 0 matches (replaced with JSON + TUS)
    - Grep `DocumentDetail.tsx` for `FormData` — 0 matches (replaced with JSON + TUS)
    - Grep `VersionDetail.tsx` for `FormData` — 0 matches (replaced with JSON + TUS)
    - Grep `Documents.tsx` for `uploadFileToStorage` — present
    - Grep `DocumentDetail.tsx` for `uploadFileToStorage` — present
    - Grep `VersionDetail.tsx` for `uploadFileToStorage` — present
  ...
  <done>
    Document upload uses TUS flow in Documents.tsx, DocumentDetail.tsx, and VersionDetail.tsx (no multer/FormData anywhere). README documents Supabase + Vercel architecture. Codebase has zero legacy references. Full build passes.
  </done>
</task>
