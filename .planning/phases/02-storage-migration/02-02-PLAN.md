...
must_haves:
  ...
  artifacts:
    ...
    - path: "client/src/lib/storage.ts"
      ...
      exports: ["uploadFileToStorage", "validateFile", "ACCEPTED_FILE_EXTENSIONS"]
  key_links:
...
<task type="auto">
  ...
  <action>
    ...
    **Functions (exported):**
    ...
    2. `uploadFileToStorage(options: UploadOptions): Promise<void>`
       ...
       - Endpoint: derive from `import.meta.env.VITE_SUPABASE_URL` — replace the hostname to get the storage hostname: `${supabaseUrl.replace('.supabase.co', '.storage.supabase.co')}/storage/v1/upload/resumable`
         - NOTE: If VITE_SUPABASE_URL contains a project ref like `https://xyz.supabase.co`, the storage URL is `https://xyz.storage.supabase.co/storage/v1/upload/resumable`
    ...
    **Important:** This module does NOT create any React components or UI. It provides the functions that Phase 4 components will call. The progress callback (`onProgress`) is the hook for progress bar UI.
  </action>
  <verify>
    - `tus-js-client` appears in package.json dependencies
    - `client/src/lib/storage.ts` exists and exports `uploadFileToStorage`, `validateFile`, `formatFileSize`, `ACCEPTED_FILE_EXTENSIONS`, `ACCEPTED_MIME_TYPES`, `MAX_FILE_SIZE_BYTES`
  ...
  <done>
    tus-js-client installed. client/src/lib/storage.ts provides: validateFile (size + type checks with clear messages), uploadFileToStorage (TUS resumable upload with progress callback), formatFileSize utility, and all relevant constants. Ready for Phase 4 UI components to consume.
  </done>
</task>
